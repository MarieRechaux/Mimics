...
if($_POST){
    ...
    if(iconv_strlen($_POST['firstName']) < 3){
        $errorFirstName = '<small class="text-danger">Format invalide. Minimum 3 caractères</small>';
        $error = true;
    }

    // 5. Contrôler la validiter de l'email (filter_var)
    // Si l'email saisi N'EST PAS (!) au bon format, alors on entre dans le IF

    /*
        function filter_var($value, type_filtre){
            // traitement
        }
    */
    ...
    // SI la taille du code postal est différente de 5 OU que le code postal N'EST PAS (!) de type numéric, alors on entre dans le IF
    if(iconv_strlen($_POST['zipcode']) != 5 || !is_numeric($_POST['zipcode'])){
        $errorZipCode = '<small class="text-danger">Format invalide. Ex: 78000</small>';
        $error = true;
    }

    // 7. Faire le même contrôle avec le numéro de téléphone
    if(iconv_strlen($_POST['phone']) != 10 || !is_numeric($_POST['phone'])){
        $errorPhone = '<small class="text-danger">Format invalide. Ex: 0664124578</small>';
        $error = true;
    }
    ...
    // Si la variable $error N'EST PAS (!) définit (isset), alors l'internaute a correctement rempli le formulaire, il n'est entré dans aucun des cas de contrôles ci-dessus, alors on entre dans le IF
    if(!isset($error)){
        $msgValidation = '<div class="alert alert-success text-center my-3">Félicitation ! t\'as tout bien rempli comme il fallait !!</div>';
    ...
    <div class="col-6 mx-auto">
        ...
        <form method="post" action="">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="zipcode" class="form-control <?php if(isset($errorZipCode)) echo $border; ?>" placeholder="Saisir votre code postal" id="zipcode" maxlength="5">
