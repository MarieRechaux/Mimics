...
if(isset($_GET['action']) && $_GET['action'] == 'update'){
    // echo "j'ai cliqué sur le bouton de modification";
    ...
    // rowCount() : méthode PDOStatement qui retourne le nombre de ligne de la table SQL séléctionnées
    // si $data->rowCount() retourne un integer, la condition IF retourne TRUE, on entre dans le IF  
    // si $data->rowCount() retourne un boolean FALSE, la condition IF retourne FALSE, on n'entre dans le IF 
    if(!$data->rowCount()){
...
// si l'indice 'submit' est définit dans la superglobal $_post, c'est que l'utilisateur a cliqué sur le bouton de validation submit du formulaire
if(isset($_POST['submit'])){
    ...
    $data->execute(); // C'est ici que la requete SQL est veritablement éxecutée en BDD

    // echo '<pre>'; print_r($data); echo '</pre>';
    // echo '<pre>'; print_r(get_class_methods($data)); echo '</pre>';

    // Après l'execution de la requete d'insertion, on redirige l'utilisateur vers la page affichage_annuaire (+ params dans l'URL, ?action=valid)
    header('Location: affichage_annuaire.php?action=valid');
}
    ...
    <div class="col-6 mx-auto">
        ...
        <form method="post" action="">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="profession" class="form-control" placeholder="Saisir votre profession" id="profession" value="<?php if(isset($arrayAnnuaire['profession'])) echo $arrayAnnuaire['profession'] ?>">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control" id="description" rows="7" ><?php if(isset($arrayAnnuaire['description'])) echo $arrayAnnuaire['description'] ?></textarea>
            </div>
